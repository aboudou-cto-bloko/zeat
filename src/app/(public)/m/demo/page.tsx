...
export default function DemoPage() {
  ...
  const updateQty = (name: string, delta: number) => {
    setCart((prev) => prev.map((i) => i.name === name ? { ...i, quantity: i.quantity + delta } : i).filter((i) => i.quantity > 0));
  };
                ...
                <div className="shrink-0">
                  {inCart ? (
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-caption font-bold w-5 text-center">{inCart.quantity}</span>
                      <button
                        onClick={() => updateQty(item.name, 1)}
